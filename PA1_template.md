...
remain under-utilized both because the raw data are hard to obtain and there is a lack
...
2. Make a panel plot containing a time series plot (i.e. ???????? = "??") of the 5-minute interval (x-axis) and the average number of steps taken, 
...
ggplot(IntervalDT , aes(x = interval , y = steps, color=`weekday or weekend`)) + geom_line() + labs(title = "Avg. Daily Steps by Weektype", x = "Interval", y = "No. of Steps") + facet_wrap(~`weekday or weekend`, nrow = 2, ncol = 1)
```
